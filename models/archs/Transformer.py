...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias) # dim = C (enbedding dim)
    ...
    def forward(self, x):
        B, N, C = x.shape
        #* 过一层FC (B, N, 3, H, C//H) -> (3, B, H, N, C//H)
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4) 
        q, k, v = qkv[0], qkv[1], qkv[2]   # make torchscript happy (cannot use tensor as tuple) #* B, H, N, C//H each
...
class PCTransformer(nn.Module):
    ...
    def __init__(self, in_chans=3, 
        ...
        # * EdgeConv
        self.grouper = DGCNN_Grouper(in_chans)  # B 3 N to B C(3) N(128) and B C(128) N(128)
        ...
        self.Mlp_latentcode = nn.Sequential(
            nn.Linear(1024, 512),
            nn.LeakyReLU(negative_slope=0.2),
            nn.Linear(512, 256)
        )
    ...
    def forward(self, inpc):
        ...
        # build point proxy
        bs = inpc.size(0)
        coor, f = self.grouper(inpc.transpose(1,2).contiguous())  # shape B C(3) N(2048) -> [B, 3, 128], [B, 128, 128]
        
        knn_index = get_knn_index(coor)
        pos =  self.pos_embed(coor).transpose(1,2) #  [B, 3, 128] -> [B, 384, 128]
        x = self.input_proj(f).transpose(1,2) #  [B, 128, 128] -> [B, 384, 128] [B,C,N(128)]

        # * encoder 这里用 ATTENTION
        for i, blk in enumerate(self.encoder):
            if i < self.knn_layer:
                x = blk(x + pos, knn_index)   # [B, 384, 128] 不变
            else:
                x = blk(x + pos)

        global_feature = self.increase_dim(x.transpose(1,2))  # 384 128 -> B 1024 128 写死的1024  # Todo 改成256 或者 加一层
        ...
        return global_feature_out, categ_prediction


class PCTransformerLocal(nn.Module):
    """ Vision Transformer with support for point cloud completion
    """
    ...
    def __init__(self, in_chans=3,
        ...
        self.category_pred = category_pred
        ...
        self.increase_dim = nn.Sequential(
            nn.Conv1d(embed_dim, output_channel, 1),
    ...
    def forward(self, inpc):
        ...
        '''
        # build point proxy
        bs = inpc.size(0) # b, N_pc, 4:xyz+category
        coor, f = self.grouper(inpc.transpose(1, 2).contiguous())  # shape B C(3) N(2048) -> [B, 3(4 if category), 128], [B, 128, 128]

        knn_index = get_knn_index(coor) # 2048
        pos = self.pos_embed(coor).transpose(1, 2)  # [B, 3, 128] -> [B, 384, 128]
        x = self.input_proj(f).transpose(1, 2)  # [B, 128, 128] -> [B, 384, 128] [B,C,N(128)]
