...
class HierarchicalAggregation(Function):
    ...
    def forward(ctx, semantic_label, coord_shift, ball_query_idxs, start_len, batch_idxs, training_mode, using_set_aggr):
        ...
        PG_OP.hierarchical_aggregation(semantic_label, coord_shift, batch_idxs, ball_query_idxs, start_len, 
...
class CalIoUAndMasklabel(Function):
    ...
    def forward(ctx, proposals_idx, proposals_offset, instance_labels, instance_pointnum, mask_scores_sigmoid, mode):
        ...
        PG_OP.cal_iou_and_masklabel(proposals_idx, proposals_offset, instance_labels, instance_pointnum, proposals_iou, nInstance, nProposal, mask_scores_sigmoid, mask_label, mode)
...
class Voxelization_Idx(Function):
    ...
    def forward(ctx, coords, batchsize, mode=4):
        ...
        :param dimension: int
        :return: output_coords:  long (M, dimension + 1) (M <= N), 去重之后的体素空间坐标点，shape为(M,3)或者(M,4)。
        :return: output_map: int M * (maxActive + 1), shape为(M,k)，列出在M_{i}体素中，含有的coords索引，k为单个体素中含有最大的点个数，由此得知，output_map含有很多0，因为有的体素含有的点个数远远小于k
        :return: input_map: int N, shape为(N)，标记第N_{i}个点，所在的体素编号，0~M-1
        '''
